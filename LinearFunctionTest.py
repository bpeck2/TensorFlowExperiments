...
def begin_training(xvalues, yvalues, num_epochs):
    # Begin training the model, set to a default 1000 iterations (epochs)
    model.fit(xvalues,yvalues, epochs=num_epochs)
...
def predict_y_values():
    ...
    return Y_VALUE_FINAL_ARRAY


def plotPrediction(Y_VALUE_FINAL_ARRAY):
...
# Begin training
model = begin_training(xarr, yarr, 1000)

# User Inputted Values
INPUT_VAL = np.array([10.0,15.0,20.0])

# Test User Input Values against our trained model
PREDICT_VALUES = model.predict([INPUT_VAL])


Y_VALUE_FINAL_ARRAY = predict_y_values()
plotPrediction(Y_VALUE_FINAL_ARRAY)
